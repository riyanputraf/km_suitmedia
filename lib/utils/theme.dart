import 'package:flutter/material.dart';

Color blueBtn = Color(0xff2B637B);
Color dark = Color(0xff04021D);
Color grey = Color(0xff686777);
Color white = Color(0xffFFFFFF);


//Textstyle
TextStyle hintStyle = TextStyle(color: grey.withOpacity(0.36), fontWeight: FontWeight.w500, fontSize: 16);
TextStyle regular = TextStyle(color: dark, fontWeight: FontWeight.w400, fontSize: 12);
TextStyle medium = TextStyle(color: white, fontWeight: FontWeight.w500);
TextStyle mediumTitle = TextStyle(color: dark, fontWeight: FontWeight.w500, fontSize: 16);
TextStyle semiBoldTitle = TextStyle(color: dark, fontWeight: FontWeight.w600, fontSize: 24);
TextStyle semiBoldSubTitle = TextStyle(color: dark, fontWeight: FontWeight.w600, fontSize: 18);
TextStyle subTitleGrey = TextStyle(color: grey, fontWeight: FontWeight.w500);
